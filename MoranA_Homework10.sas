...
imported raw data into SAS manually via the DATALINES statement. The only 
other type of data we've seen are permanent SAS data sets, which we
imported with a SET statement.
...
above. 
-Change the informat on the description variable so that it is no longer cut off. 
...
*****************************************************************************/
data icecream2;
input name $16. size cals fat desc $26. test1-test5 price comma5.2;
...
proc means data = icecream2 mean;
	var price;
run;
